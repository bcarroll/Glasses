	...
	class BluetoothChemion {
		...
		saveToSlot(slot, frames) {
			return new Promise((resolve, reject) => {
				...
				// Wait for the START + frame bytes to be physically written, then give the
				// glasses' firmware a moment to finish processing the last frame before
				// sending END. Testing showed sending END immediately after the frame data
				// gets rejected with ERROR code 1 - a "not ready yet" response, not a
				// malformed request (START itself is acknowledged cleanly).
				this._whenIdle().then(() => {
					setTimeout(() => {
						if (this._PROMISES.saveSlot !== pending) return; // already rejected by an earlier ERROR
		...
		_handleReply(payload) {
			...
			switch (command) {
				...
				case CHEMION_COMMAND.FRAMES_TRANSMISSION_START:
					console.log('BluetoothChemion: glasses acknowledged FRAMES_TRANSMISSION_START - payload: ' + toHex(payload));
					break;
				...
				case CHEMION_COMMAND.FRAMES_TRANSMISSION_END:
					...
					} else if (this._PROMISES.saveSlot) {
						console.log('BluetoothChemion: glasses acknowledged FRAMES_TRANSMISSION_END - payload: ' + toHex(payload));
						this._PROMISES.saveSlot.resolve();
					}
