	...
	class BluetoothChemion {
		...
		_onNotify(chunk) {
			...
			while (true) {
				...
				if (start > 0) {
					console.log('BluetoothChemion: discarding ' + start + ' byte(s) before next 0xfa marker', this._RX_BUFFER.slice(0, start));
				...
				try {
					...
					if (reply.type === CHEMION_TYPE.REPLY || reply.type === CHEMION_TYPE.STREAM) {
						...
						console.log('BluetoothChemion: notification with unhandled message type 0x' + reply.type.toString(16), reply.payload);
					}
				}
				catch (error) {
					console.log('Could not decode message! ' + error, messageBytes);
				}
			}
		}
